...
function Stepper({ step }) {
  ...
  return (
    <>
      {/* Mobile Stepper Card */}
      <div className="bg-white rounded-2xl border border-border p-3.5 sm:p-4 mb-6 md:hidden shadow-2xs">
          ...
          {[1, 2, 3].map((num) => (
            <div 
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                step >= num ? 'bg-maroon' : 'bg-border/60'
              }`}
            />
          ))}
        </div>
        {/* Step node indicators */}
        <div className="flex items-center justify-between text-[10.5px] font-semibold text-text-muted px-1">
          {STEPS.map((label, i) => {
            const num = i + 1
            ...
            return (
              <span 
                ...
                className={`transition-colors flex items-center gap-1 ${
                  active ? 'text-maroon font-bold' : done ? 'text-text-main' : 'text-text-muted'
                }`}
              >
                <span className={`w-4 h-4 rounded-full inline-flex items-center justify-center text-[9px] font-bold ${
                  done ? 'bg-maroon text-white' : active ? 'bg-maroon-light text-maroon border border-maroon-border' : 'bg-off-white text-text-muted'
                }`}>
                  {done ? '✓' : num}
                </span>
                {label}
    ...
    const successView = (
      <div className="flex-1 flex flex-col items-center justify-center p-3 sm:p-6 text-center">
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-border p-5 sm:p-8 md:p-10 max-w-md w-full text-center shadow-lg animate-fade-up">
          ...
          <p className="text-sm sm:text-base font-bold text-text-main m-0 mb-1">{selectedType?.name}</p>
          <p className="text-xs sm:text-sm text-text-sub m-0 mb-4 sm:mb-6 flex items-center justify-center gap-1.5">
            <Calendar size={14} className="text-gold shrink-0" /> {fmtDate(selectedDate)} at {fmt12h(selectedSlot)}
          </p>
          <div className="bg-gold-light rounded-xl py-3 px-3.5 mb-4 sm:mb-6 border border-gold-border text-left">
            <p className="text-xs sm:text-[13px] text-gold m-0 font-medium leading-normal flex items-start gap-1.5">
              <span className="shrink-0 font-bold">ℹ</span>
              <span>Please bring all required physical documents on your appointment date.</span>
            </p>
          </div>
          ...
          <div className="flex-1 md:min-w-0 md:bg-white md:p-8 md:rounded-3xl md:border md:border-border md:shadow-sm">
                  ...
                  {types.map(t => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Bottom: Requirements Badges */}
                        <div className="w-full pt-2.5 sm:pt-3 border-t border-border/50 mt-auto flex flex-wrap gap-1 sm:gap-1.5 items-center">
                          {reqDocs.length > 0 ? (
                            reqDocs.map((doc, j) => (
                              <span key={j} className="text-[10px] sm:text-[11px] text-text-sub bg-off-white py-0.5 sm:py-1 px-2 sm:px-2.5 rounded-lg border border-border font-medium">
                                {doc}
                              </span>
                            ))
                          ) : (
                            <span className="text-[10px] sm:text-[11px] text-text-muted italic py-0.5">
            ...
            {step === 2 && selectedType && (
              <div className="animate-fade-up">
                ...
                {/* Selected Document Pill Banner (Mobile Context) */}
                <div className="flex items-center justify-between p-2.5 sm:p-3 rounded-2xl bg-white border border-border mb-4 shadow-2xs">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-7 h-7 rounded-lg bg-maroon text-white flex items-center justify-center shrink-0">
                      <FileText size={14} />
                    </div>
                    <div className="min-w-0">
                      <span className="text-[9.5px] font-extrabold uppercase tracking-widest text-maroon block">Selected Document</span>
                      <span className="text-xs sm:text-sm font-bold text-text-main truncate block">{selectedType.name}</span>
                    </div>
                  </div>
                  <button 
                    type="button" 
                    onClick={() => { setStep(1); setSelectedType(null); setSelectedDate(''); setSelectedSlot(''); setSlotsData(null) }}
                    className="text-xs font-bold text-maroon hover:text-maroon-dark bg-off-white py-1 px-2.5 rounded-lg border border-border shadow-2xs hover:bg-maroon-light cursor-pointer shrink-0 transition-colors ml-2"
                  >
                    Change
                  </button>
                </div>
                ...
                {needsAcademicInfo && (
                  <div className="bg-white rounded-2xl border border-border sm:border-[1.5px] p-4 sm:p-6 mb-4 shadow-2xs">
                    ...
                    <p className="text-[11px] sm:text-xs text-text-sub m-0 mb-3">Please provide the following academic details for your request.</p>
                ...
                <div className="flex justify-between items-center gap-3 mt-4 pt-2">
                  <button 
                    ...
                    className="py-2.5 px-4 sm:py-3 sm:px-6 rounded-xl border border-maroon-border bg-white text-maroon text-xs sm:text-sm font-bold cursor-pointer font-sans hover:bg-maroon-light transition-colors shadow-2xs"
                  >
                    ← Back
                  </button>
                  <button 
                    ...
                    className={`py-2.5 px-4 sm:py-3 sm:px-6 rounded-xl border-none text-xs sm:text-sm font-bold font-sans transition-all shadow-2xs ${
              ...
              <div className="animate-fade-up">
                <div className="text-center mb-4 sm:mb-6">
                  ...
                  <p className="text-xs sm:text-sm text-text-sub m-0">
                    Please verify your details before confirming your visit.
                  </p>
                </div>
                ...
                <div className="bg-white rounded-2xl sm:rounded-3xl border border-border sm:border-[1.5px] overflow-hidden shadow-xs mb-4">
                  ...
                  <div className="p-4 sm:p-6 md:p-7">
                    ...
                    <div className="mb-4 pb-4 sm:mb-6 sm:pb-5 border-b border-border">
                      <p className="text-[10px] font-extrabold text-text-muted tracking-widest uppercase m-0 mb-2">TRANSACTION</p>
                      <div className="flex items-start gap-3">
                        <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-maroon-light text-maroon flex items-center justify-center shrink-0 border border-maroon-border/40">
                          <FileText size={18} />
                        </div>
                        <div className="min-w-0">
                          <h3 className="font-serif text-sm sm:text-lg font-bold text-text-main m-0 mb-1 truncate">{selectedType.name}</h3>
                    ...
                    {/* Schedule + Location */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 mb-4 pb-4 sm:mb-6 sm:pb-5 border-b border-border">
                      ...
                      <div className="p-3.5 sm:p-4 rounded-xl bg-white border border-border shadow-2xs">
                        ...
                        <div className="flex items-start gap-2 text-xs sm:text-sm text-text-main">
                          ...
                          <div>
                            ...
                            <div className="text-[11.5px] text-text-sub mt-0.5">CRMC Upper Pandan Campus</div>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Requirements */}
                    <div>
                      <p className="text-[10px] font-extrabold text-text-muted tracking-widest uppercase m-0 mb-2.5">REQUIREMENTS TO BRING</p>
                ...
                <div className="text-[11.5px] sm:text-xs text-text-muted mb-4 sm:mb-6 text-center flex items-center justify-center gap-1.5">
                  <Mail size={13} className="shrink-0" /> A confirmation email will be sent to your student email.
                </div>
                ...
                <div className="flex justify-between items-center gap-3">
                  <button 
                    ...
                    onClick={() => setStep(2)}
                    disabled={loading}
                    className={`py-2.5 px-4 sm:py-3 sm:px-6 rounded-xl border border-maroon-border bg-white text-maroon text-xs sm:text-sm font-bold font-sans shadow-2xs ${loading ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-maroon-light'}`}
                  >
                    ← Back
                  </button>
                  <button 
              ...
              <div className="fixed inset-0 z-1000 flex items-center justify-center p-4">
                ...
                <div className="animate-fade-up relative w-[92%] max-w-85 bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-6 text-center shadow-2xl">
                  ...
                  <p className="text-xs sm:text-[13px] text-text-sub m-0 mb-5 leading-relaxed">
                    Are you ready to confirm your appointment for <strong className="text-text-main">{fmtDate(selectedDate)}</strong> at <strong className="text-maroon">{fmt12h(selectedSlot)}</strong>?
                  </p>
